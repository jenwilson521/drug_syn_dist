...
class MyGraph:
    ...
    def read_graph(self,
        ...
        if file_type == 'edge_list':
            graph_df = pd.read_csv(file_name, delimiter=separator, comment=comment)
            

            graph_df = graph_df.iloc[:, 0:3] 
            ...
            ori_src = graph_df.iloc[:, 0].values
            ori_dst = graph_df.iloc[:, 1].values
            ...
            if self._weighted:
                weights = graph_df.iloc[:, 2].values
            else:
    ...
    def sweep_with_conductance(self, score, window=3):
        ...
        #print(score[4287])
        #print(score[6182])
        #print(self._dict_ori_node_id_to_vir_node_id)
        ...
        #Tried degree normalization...
        #N = self._num_vertices
        #g = 3
        #sorted_nodes = sorted(sorted_nodes, key=lambda tmp_node: score[tmp_node] / ((10**g * self._degree[tmp_node]) / (N - 1)), reverse=True)
        sorted_nodes = sorted(sorted_nodes, key=lambda tmp_node: score[tmp_node], reverse=True)
        #for sn in sorted_nodes[:100]:
        #    print(score[sn], sn, self._dict_vir_node_id_to_ori_node_id[sn])
        ...
        for cur_node in sorted_nodes:
            ...
            if conductance < best_conductance:
                ...
                #print('SWEEP SET:', best_sweep_set, best_conductance)
                increase_count = 0
            else:
                increase_count += 1
